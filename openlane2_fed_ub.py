...
def get_package_manager():
    try:
        check_output(['apt', '--version'])
        return "apt"
    except Exception:
        pass
    try:
        check_output(['dnf', '--version'])
        return "dnf"
    except Exception:
        pass
    return None
...
# install required dependencies based on distro
def install_prereqs():
    pkg_mgr = get_package_manager()
    if pkg_mgr is None:
        print("Unsupported system: neither apt nor dnf found.")
        return
    ...
    try:
        if pkg_mgr == "apt":
            ...
            run(['sudo', 'apt', 'install', '-y'] + base_packages, check=True)
        elif pkg_mgr == "dnf":
            run(['sudo', 'dnf', 'install', '-y'] + base_packages, check=True)
        ...
        try:
            ...
            try:
                if pkg_mgr == "apt":
                    ...
                    print("Unsupported distro for curl installation.")
                    return
            except Exception as e:
